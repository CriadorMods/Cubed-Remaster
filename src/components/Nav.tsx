import React from "react";
import { useNavigate, useLocation } from "react-router-dom";

// Icons
import { FaBars } from "react-icons/fa";
import { SlMagnifier } from "react-icons/sl";
import { BsGlobe } from "react-icons/bs";
import { FaUserLarge } from "react-icons/fa6";
import { RiCloseLargeFill } from "react-icons/ri";

// Assets
import Fortnite from "@/assets/Icons/Fortnite.png";

// Zustand
import { useModalsStore } from "@/zustand/ModalStore";

const Nav: React.FC = () => {
  const { setDownloads, setQuestion, setMobileSidebar, mobileSidebar } = useModalsStore();
  const navigate = useNavigate();
  const location = useLocation();

  return (
    <div className="font-OpenSans absolute top-0 left-0 w-full h-[3.2rem] z-50 bg-[#2a2a2a] flex flex-row justify-between items-center border-1 border-[#2a2a2a]">
      <div className="h-full flex justify-center items-center gap-2 pl-2">

        <img 
        src="data:image/png;base64," 
        alt="Logo"
        className="h-7.5 w-auto opacity-90 px-1.5" 
        />

        <img src={Fortnite} alt="Logo" className="w-24.5 h-7.5 ml-3" />

        <div className="flex h-full flex-row ml-1 md:ml-6 uppercase text-[#ccc]">
          <div
            onClick={() => navigate("/")}
            className={`
                relative px-3.5 hidden md:flex font-bold hover:text-white/95 items-center text-[0.7rem] cursor-pointer after:absolute 
                after:bottom-0 after:left-0 after:w-full after:h-0 after:bg-[#ffff00] hover:after:h-[5px] after:transition-all 
                after:duration-250 overflow-hidden tracking-wide 
                ${
                  location.pathname === "/" ? "after:h-[5px] text-white/90" : ""
                }
                `}
          >
            Get Fortnite
          </div>

          <div
            onClick={() => navigate("/battle-pass/cubed")}
            className={`
                relative px-3.5 hidden md:flex font-bold hover:text-white/95 items-center text-[0.7rem] cursor-pointer after:absolute 
                after:bottom-0 after:left-0 after:w-full after:h-0 after:bg-[#ffff00] hover:after:h-[5px] after:transition-all 
                after:duration-250 overflow-hidden tracking-wide 
                ${
                  location.pathname === "/battle-pass/cubed" ? "after:h-[5px] text-white/90" : ""
                }
                `}
          >
            Battle Pass
          </div>

          <div
            onClick={() => navigate("/news")}
            className={`
                relative px-3.5 hidden md:flex font-bold hover:text-white/95 items-center text-[0.7rem] cursor-pointer after:absolute 
                after:bottom-0 after:left-0 after:w-full after:h-0 after:bg-[#ffff00] hover:after:h-[5px] after:transition-all 
                after:duration-250 overflow-hidden tracking-wide
                ${
                  location.pathname.includes("/news") ? "after:h-[5px] text-white/90" : ""
                }
                `}
          >
            News
          </div>

          <div
            onClick={() => navigate("/flux/competitive")}
            className={`
                relative px-3.5 hidden md:flex font-bold hover:text-white/95 items-center text-[0.7rem] cursor-pointer after:absolute 
                after:bottom-0 after:left-0 after:w-full after:h-0 after:bg-[#ffff00] hover:after:h-[5px] after:transition-all 
                after:duration-250 overflow-hidden tracking-wide
                ${
                  location.pathname.includes("/flux/competitive") ? "after:h-[5px] text-white/90" : ""
                }
                `}
          >
            Competitive
          </div>
        </div>
      </div>
      <div className="h-full flex text-[#ccc]">
        <div
          className="relative px-4 hidden md:flex hover:text-white/95 items-center text-[0.7rem] transition cursor-pointer overflow-hidden tracking-wide"
        >
          <SlMagnifier className="text-lg"/>
        </div>
        <div
          className="relative px-4 hidden md:flex hover:text-white/95 items-center text-[0.7rem] transition cursor-pointer overflow-hidden tracking-wide"
        >
          <BsGlobe className="text-lg"/>
        </div>

        { location.pathname !== "/register" && location.pathname !== "/sign-in" &&
        <>
        <div
        onClick={() => setQuestion(true)} 
        className="relative px-4 hidden md:flex gap-2 hover:text-white/95 items-center text-[0.7rem] transition cursor-pointer overflow-hidden uppercase tracking-wide"
        >
          <FaUserLarge className="text-lg"/>
          Sign In
        </div>
        
        <button 
        onClick={() => setDownloads(true)}
        className="h-full uppercase xl:px-9 hidden sm:flex bg-[#ffff00] hover:bg-[#f5f500] px-6 font-medium text-black/90 text-[10px] items-center cursor-pointer transition-all">
          Download
        </button>
        </>
        }

        <button 
        onClick={() => setMobileSidebar(!mobileSidebar)}
        style={{ background: mobileSidebar ? "#252525" : "", color: mobileSidebar ? "#e7e7e7" : "" }}
        className="h-full uppercase flex sm:hidden bg-[#ffff00] hover:bg-[#f5f500] px-4 font-medium text-black/90 text-[24px] items-center cursor-pointer transition-all">
          {mobileSidebar ? <RiCloseLargeFill /> : <FaBars />}
        </button>
      </div>
    </div>
  );
};

export default Nav;